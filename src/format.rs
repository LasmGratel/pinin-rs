...
const OFFSET: &[&'static str] = &["ui", "iu", "uan", "uang", "ian", "iang", "ua", "ie", "uo", "iong", "iao", "ve", "ia"];
...
lazy_static! {
    ...
    static ref TONES: Vec<&'static HashMap<char, char>> = Vec::from([&*NONE, &*FIRST, &*SECOND, &*THIRD, &*FOURTH]);
...
pub fn raw_format<'a>(p: &'a Pinyin) -> Cow<'a, str> {
    Cow::Borrowed(&p.raw[0..(p.raw.len() - 1)])
}
...
pub fn phonetic_format<'a>(p: &'a Pinyin) -> Cow<'a, str> {
    ...
    if let Some(str) = LOCAL.get(&s[0..s.len()-1]) {
        let c = s.chars().last();
        s = str.to_string();
        if let Some(c) = c {
            s.push(c);
    ...
    let split = if Pinyin::has_initial(&s) {
        ["", &s[0..len-1], &s[len-1..len]]
    } else {
        let i = if len > 2 && s.chars().skip(1).next() == Some('h') { 2 } else { 1 };
        [&s[0..i], &s[i..len-1], &s[len-1..len]]
    };
...
pub fn unicode_format<'a>(p: &'a Pinyin) -> Cow<'a, str> {
    ...
    let finale = if Pinyin::has_initial(s) {
        let i = if s.len() > 2 && s.chars().skip(1).next() == Some('h') { 2 } else { 1 };
        ret.push_str(&s[0..i]);
        &s[i..s.len()-1]
    } else {
        &s[0..s.len()-1]
    ...
    if offset == 1 {
        ret.push_str(&finale[0..1]);
    }
    let group = TONES[s.chars().last().and_then(|c| c.to_digit(10)).map(|x| x as usize).unwrap_or(0)];
    if let Some(c) = finale.chars().skip(offset).next() {
        if let Some(tone) = group.get(&c) {
            ret.push(*tone);
        }
    }
    if finale.len() > offset + 1 {
        ret.push_str(&finale[offset+1..offset+1+finale.len()]);
    }
